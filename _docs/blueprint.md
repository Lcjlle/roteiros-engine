...
- **Concordância entre anotadores:** **simpledorff** (linha própria abaixo — NÃO ADOTADO, motivo técnico, não de licença), a lib **krippendorff** (linha abaixo, NÃO ADOTADO — GPL-3.0), e **nltk.metrics.agreement** (Cohen/Fleiss kappa). **Resolvem:** medir se o LLM-anotador concorda com o humano acima do limiar aceitável — os valores exatos vivem em `schema/portoes.json` (`fase5-model-human-agreement-alpha`, model×human; `fase4-self-agreement-alpha`, human×human 48h), não repetidos aqui (Regra #22). Este é o seu portão de qualidade para dizer "o perfil é confiável".
...
- **NÃO ADOTADO — motivo técnico, não de licença:** **LightTag/simpledorff** — https://github.com/LightTag/simpledorff — função de conveniência que calcula o alpha de Krippendorff sobre um DataFrame pandas (`calculate_krippendorffs_alpha_for_df(df, experiment_col, annotator_col, class_col, metric_fn=nominal_metric)`); `metric_fn` aceita qualquer função pura de dois argumentos, mesmo formato do `distance` de `nltk.metrics.agreement.AnnotationTask` (confirmado lendo `simpledorff/simpledorff.py` e `simpledorff/metrics.py` no repositório: só `nominal_metric`/`interval_metric` vêm prontas, mas o parâmetro não é travado a um enum fixo). Licença **MIT** (confirmada no `LICENSE` do repositório e no classifier do PyPI) — **não é problema de licença**. **Por que saiu:** (1) o projeto já adotou `nltk.metrics.agreement.AnnotationTask` como seu único ponto de entrada de concordância (`_docs/decisions.md#28(d)`, linha acima), cobrindo o caso nominal que os outros quatro campos da ontologia precisam e o caso ordinal de `density` via a closure implementada pelo próprio projeto — uma segunda lib só para o caso Krippendorff-alpha isolado seria redundante; (2) `simpledorff/data_transforms.py` importa `pandas` incondicionalmente — dependência nova que este projeto não usa em nenhum outro lugar (`pandas` não está em `pyproject.toml`) — enquanto `AnnotationTask` opera sobre a mesma lista de triplas `(coder, item, label)` que a exclusão de dado ausente de `evidence_type` (`_docs/decisions.md#28(e)`) já constrói, sem dependência nova.
...
**Estágio 2 — Anotação assistida por LLM + validação.** Use `instructor` para rotular o resto do corpus contra a ontologia (JSON validado, `temperature` baixa, 3 execuções). Meça a concordância LLM×humano com `nltk.metrics.agreement.AnnotationTask` (`simpledorff` não foi adotado — ver Peça 5). **[REVISTO v2.0]** Meça **no nível de janela**, nunca depois da fusão em blocos: a fusão apaga discordâncias e infla o número. E não passe a posição percentual da janela no prompt — o modelo responderia pela posição e não pelo texto, e o perfil confirmaria a estrutura que você mesmo injetou. O volume real é de ~3.600 chamadas, não 1.350, então o runner precisa ser retomável. *Benchmark decisivo:* o portão `fase5-model-human-agreement-alpha` (`schema/portoes.json`), por campo. Abaixo do limiar, o rótulo automático não é confiável — melhore as definições da ontologia (não o modelo) e repita. Este é o gatilho que muda tudo: só agregue o perfil quando passar o limiar.
