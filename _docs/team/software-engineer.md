...
own `.venv`. The orchestrator sets it up and tells you where it is.
...
- If the orchestrator rebases your branch, stop pushing it. Commit, say in
  ...
  Never force-push
- Other issues may be building the corpus, gold set, or profile for the same
  channel at the same time. If a file under `corpus/`, `gold/`, or
  `perfis/` you need does not exist yet, it belongs to an issue that has not
  merged - build against what your issue tells you to assume, not against
  their branch
...
- Delete nothing. Undo an edit with `git checkout -- <path>`, put scratch
  ...
  generated corpus/gold/output files alone unless the issue is specifically
  about removing them
