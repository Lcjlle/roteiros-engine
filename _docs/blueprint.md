...
- **pln-fing-udelar/fast-krippendorff** — https://github.com/pln-fing-udelar/fast-krippendorff — implementação rápida do alpha de Krippendorff (`krippendorff.alpha(reliability_data=..., level_of_measurement=...)`), incluindo o nível `ordinal` que uma escala censurada como `density` (`schema/ontologia.v1.json`) exige. Licença **GPL-3.0** (confirmada no `LICENSE.txt` do repositório e no classifier do PyPI — diferente de todo o resto do núcleo permissivo desta lista). Citada antes só de passagem dentro do bullet de "Concordância entre anotadores" (linha acima, sem licença nomeada para nenhuma das três libs ali); linha própria adicionada quando `_docs/decisions.md#28(d)` decide adotá-la, especificamente para o α de `density` — mesmo processo do incidente do Pydantic desta rodada (lib usada antes de ter linha própria de licença checada). Copyleft não bloqueia o uso atual: `DECISOES.md` item 3 fixa uso não comercial e a lib nunca é distribuída como parte de um produto, só roda internamente na validação da Fase 5 — mas revisar de novo se `roteiros-engine` (o pipeline, não os roteiros gerados) for algum dia distribuído.
...
- **Licenças a evitar para uso comercial:** `TextMachina` (Genaios) é **CC-BY-NC-ND-4.0 — não comercial e sem derivados**; `gpt_annotate` e `core-stories` **não têm arquivo de licença** (por padrão, todos os direitos reservados); `ScreenPy` tem licença ambígua; ferramentas em **R** como `stylo`/`syuzhet` são GPL; a lib Python **`krippendorff`** (Peça 5) também é **GPL-3.0** — mesma família de cautela, mas não bloqueia o uso interno e não comercial deste projeto (ver a linha própria dela acima). Verifique o arquivo LICENSE antes de embutir qualquer um em produto pago.
